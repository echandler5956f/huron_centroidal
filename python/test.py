...
def barycentric_interpolator(x, y, z, xi, yi):
    """
    2D Barycentric Interpolator function using CasADi.

    Parameters:
    - x, y: Arrays of x and y coordinates of the data points.
    - z: Array of function values at the corresponding (x, y) points.
    - xi, yi: Symbolic variables for the interpolation points.

    Returns:
    - zi: Symbolic expression for the interpolated values at (xi, yi).
    """

    # Convert input data to CasADi SX (symbolic) variables
    x = ca.SX(x)
    y = ca.SX(y)
    z = ca.SX(z)

    # Create symbolic variables for interpolation points
    xi = ca.SX(xi)
    yi = ca.SX(yi)
    ...
    # Calculate interpolated value using barycentric coordinates
    zi = ca.mtimes(w.T, z) / ca.mtimes(w.T, ca.SX(1))
...
# Define symbolic variables for interpolation points
xi_symbolic = ca.SX.sym('xi')
yi_symbolic = ca.SX.sym('yi')

# Call the barycentric_interpolator function
interpolated_value = barycentric_interpolator(x_data, y_data, z_data, xi_symbolic, yi_symbolic)

# Create a CasADi function from the symbolic expression
interpolator_function = ca.Function('interpolator', [xi_symbolic, yi_symbolic], [interpolated_value])

# Test the interpolator function with specific values
result = interpolator_function(0.3, 0.7)
